...
def change_tone(text, tone):
    """Change the tone of the text using predefined rules."""
    if tone == "formal":
        # Simple rule-based transformation for formal tone
        replacements = {
            "don't": "do not",
            "can't": "cannot",
            "won't": "will not",
        ...
        # Simple rule-based transformation for casual tone
        replacements = {
            "do not": "don't",
            "cannot": "can't",
            "will not": "won't",
...
class DocumentAnalysisApp:
    def __init__(self, root):
        ...
        self.tone_button = tk.Button(root, text="Change Tone", command=self.change_tone)
        self.tone_button.pack(pady=5)
    ...
    def change_tone(self):
        ...
        tone = simpledialog.askstring("Change Tone", "Enter the desired tone (e.g., formal, casual):")
        if tone:
            text = extract_text(self.file_path)
            updated_text = change_tone(text, tone)
            self.result_text.delete(1.0, tk.END)
            self.result_text.insert(tk.END, f"=== Tone Change ({tone}) ===\n")
            self.result_text.insert(tk.END, updated_text + "\n\n")
